...
from dynamics.quadrotors import Quadrotors2D, NearHoverQuadcopter
...
class NearHoverQuadcopterHoveringController(Controller):
    """
        This controller trying to stablize and hover 10D quadcopter at given place.
    """
    def __init__(self, dynamics:NearHoverQuadcopter, xf:np.ndarray, Q:np.ndarray, R:np.ndarray) -> None:
        super().__init__()
        ...
        if np.linalg.norm(xf[3:]) > 0:
            raise ValueError("Final Velocity or Angle is not zero")
        
        self.uf = np.array([self.dynamics.g * self.dynamics.m / self.dynamics.kT, 0, 0])

        self.A = np.vstack([np.hstack([np.zeros((5,5)), np.eye(5)]),
                np.array([0,0,0,self.dynamics.g,0, 0,0,0,0,0]),
                np.array([0,0,0,0,self.dynamics.g, 0,0,0,0,0]),
                np.zeros((3,10))
                ])
    
        self.B = np.vstack([np.zeros((7,3)),
                            np.array([self.dynamics.kT / self.dynamics.m, 0, 0]),
                            np.array([0, self.dynamics.n0, 0]),
                            np.array([0, 0, self.dynamics.n0])
                            ])
...
class Quadrotors2DWaypointsPlanner():
...
if __name__ == "__main__":
    ...
    nearHoverQuadcopterController =  NearHoverQuadcopterHoveringController(nearHoverQuadcopter, nearHoverQuadcopter.x0_mean, np.eye(10), np.eye(3))
    ...
    for i in range(1, t.shape[0]):
        us.append(nearHoverQuadcopterController.get_control_efforts(xs[i-1]))
